...
Note the double `==` in the statement to indicate "is equal to"! Further, as the variable of interest is a string (or character type), we need to use quotations around the city name.
...
[**Note**]{.underline}: when using the pipe operator, we no longer need to include the dataframe in the first argument of the function. Compare the previous two code chunks to see what I mean.
...
A few things to note here: after each function call, we need to include the pipe operator to tell dplyr that we're not done yet, and the chain of commands continues on the next line.
...
Next class we will be working through [a case study centred on a biomedical data set](#0).
