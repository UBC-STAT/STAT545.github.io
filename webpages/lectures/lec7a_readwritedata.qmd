...
I could always read in my `penguins` dataset using an absolute file path where the file path begins at the root of your computer (for me, it is a long chain of folders in `/Users/......../STAT545/stat454.github.io/webpages/lectures/datasets/penguins.csv`). This, however, is not best practice. As previously mentioned, this string telling me where the file path is can also differ for Windows users! I'd have to manually change all of the forward slashes to backslashes to make this run on Windows.
...
For example, the `penguins.csv` data set is located in `webpages` \> `lectures` \> `datasets` within my R project folder. I can access it by:

```{r}
penguins <- read_csv(here("webpages", "lectures", "datasets", "penguins.csv"))
